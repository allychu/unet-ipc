"""
Train a dataset on U-Net architecture - IPC variations
...
- `-m`: Set flag if training on pre-training on IPC as ground truth
...
# ───────────── HYPER-PARAM TUNING + 3-FOLD CV ─────────────
def objective(trial):
    """Optuna objective that trains a fresh model on each fold
       and returns mean validation IoU."""
    lr         = trial.suggest_loguniform("lr", 1e-5, 1e-3)
    batch_size = trial.suggest_categorical("batch_size", [4, 8, 16])

    all_imgs = sorted(os.listdir(f"{data_dir}TR/im-good"))
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(all_imgs)):
        ...
        fold_train_ds, fold_train_ld = get_dataset_and_loader(
            f"{data_dir}TR/im-good", f"{data_dir}TR/gt-good", shuffle=True)
        fold_val_ds,   fold_val_ld   = get_dataset_and_loader(
            f"{data_dir}TR/im-good", f"{data_dir}TR/gt-good")

        fold_train_ld.dataset.image_filenames = train_imgs
        fold_val_ld.dataset.image_filenames   = val_imgs

        model = UnetModel("Unet", "resnet18", in_channels=3, out_classes=1)
        trainer = pl.Trainer(
            max_epochs=3,         
        ...
        trainer.fit(model, fold_train_ld, fold_val_ld)
        metrics = trainer.validate(model, dataloaders=fold_val_ld, verbose=False)
        fold_ious.append(metrics[0]["valid_dataset_iou"])

    return np.mean(fold_ious)

study = optuna.create_study(direction="maximize")
study.optimize(objective, n_trials=10)
print("Best params:", study.best_params, "Mean IoU:", study.best_value)
